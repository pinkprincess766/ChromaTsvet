...
from python_analyzer.analysis.models import AnalysisSettings, ReferencePeak
...
class MainWindowErrorHandlingTest(unittest.TestCase):
    ...
    def test_analysis_session_save_writes_snapshot_without_private_source_path(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            with (
                patch.object(
                    main.QFileDialog,
                    "getSaveFileName",
                    return_value=(str(output_path), "ChromaTsvet Session (*.chromatsvet-session.json)"),
                ),
                patch.object(QMessageBox, "information", return_value=QMessageBox.Ok),
            ):
    ...
    def test_analysis_session_load_restores_snapshot_without_rerunning_analysis(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            with (
                patch.object(
                    main.QFileDialog,
                    "getOpenFileName",
                    return_value=(str(session_path), "ChromaTsvet Session (*.chromatsvet-session.json)"),
                ),
                patch.object(QMessageBox, "warning", return_value=QMessageBox.Ok),
            ):
